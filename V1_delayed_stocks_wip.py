...
tokens = sym['Instrument'].tolist()[:450]
...
def mailing(DF):
    ...
    s.starttls()
    s.login("manish.singh@hec.edu", "*****Heybhagwanji1*****")
    msg = MIMEMultipart()
    msg['Subject'] = f"{str((t1 +dt.timedelta(hours=5,minutes=30)).strftime('%H_%M'))} - Data"
    msg['From'] = 'manish.singh@hec.edu'
    html = """\
    <html>
      <head></head>
      <body>
...
def main(): 
    ...
    if len(final)>0:
        ...
        name = "V1_stocks_delayed_"+str(t1.strftime('%H_%M')) + ".csv"
        # final['News'] = final['index'].map(news_dic['News'])
        if len(final) >0:
            final['time'] = str((datetime.strptime(ohlc['date'].iloc[-1],"%Y-%m-%d %H:%M") +dt.timedelta(hours=5,minutes=30)).strftime('%H_%M'))
            global d
            d = d.append(final)
            d = d.sort_values(by = ['time'], ascending = [False])
            d['unique'] = d['index'] + d['time']
            d = d.drop_duplicates(['unique'], keep='first')
            d = d.drop(['unique'], axis = 1)
            d = d[['index','Action','time','gap_up','vol_blast','vwap','fifteen_cross','Boll_spike','pattern','close','OC_Resistance','dyn_resistance','OC_Support','dyn_support','Near_OC_Resistance_S','Near_OC_Support_B','rsi','last_candle','tick']]
            mailing(d)                
    return final
