...
def main(): 
    ...
    for ticker in tokens:
        try:
            ohlc = fetchOHLC(ticker,"3minute",6)
    ...
    if len(final)>0:
        ...
        # final['Break_OC_Resistance_B'] = np.where(final['close'] > final['OC_Resistance'] + final['OC_len'],1,0)
        # final['Break_OC_Support_S'] = np.where(final['close'] < final['OC_Support'] - final['OC_len'],1,0)
        ...
        name = "V1_"+str(t1.strftime('%H_%M')) + ".csv"
