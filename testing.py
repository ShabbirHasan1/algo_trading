...
def StochRSI_EMA(df, period=14, smoothK=3, smoothD=3):
    # Calculate RSI 
    delta = df["close"].diff().dropna()
    ups = delta * 0
    downs = ups.copy()
...
def StochRSI(df, period=14, smoothK=3, smoothD=3):
    ...
    return stochrsi.iloc[-1], stochrsi_K, stochrsi_D

def ema_fin(DF, n1=5, n2=20):
    df = DF.copy()
    df['MA1'] = df["close"].ewm(span=n1,min_periods=n1).mean()
    df['MA2'] = df["close"].ewm(span=n2,min_periods=n2).mean()
    avg_gap = abs(df["MA1"] - df["MA2"]).median()
    t=0
    if df["MA1"].iloc[-1] - df["MA2"].iloc[-1] > 0.1*avg_gap:
        t=1
    else:
        t=0
    return t
...
def main(): 
    ...
    for ticker in tokens:
        try:
            ...
            rsi_dic[dic1['Symbol'][ticker]],_,_ = StochRSI(ohlc)
    ...
    try:
        ...
        pass
    try:
        OC_trend = pd.read_csv("OC_trend.csv").set_index("index")
        OC_trend = OC_trend.loc[:, OC_trend.isnull().mean() < .16]
    except:
        pass
    try:
    ...
    if len(final)>0:
        ...
        if len(final) >0:
            try:
                ...
                OC_trend = OC_trend[OC_trend.index.isin(final.index)]
                final = pd.concat([final, OC_trend], axis=1).reset_index()
