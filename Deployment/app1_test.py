...
tokens = sym['Instrument'].tolist()[:480]
...
def main(): 
    ...
    for ticker in tokens:
        try:
            ohlc = fetchOHLC(ticker,"3minute",6)[:-i]
    ...
    if len(final)>0:
        ...
        final = final[final['vwap']<=0]
        final = final[final['last_candle']<=10]
        if len(final) >0:
            final['time'] = str(datetime.strptime(ohlc['date'].iloc[-1],"%Y-%m-%d %H:%M").strftime('%H_%M'))
            global d
            d = d.append(final)
            ...
            d = d[['index','Action','time','gap_up','vol_blast','vwap','fifteen_cross','Boll_spike','pattern','close','dyn_resistance','dyn_support','rsi','last_candle','tick']]
            mailing(d)                
    return final
